...
const { SUPPORTED_WORKSPACE_EXPORTS, WORKSPACES_CODEGEN_PATH, APP_PATH, WORKSPACES, } = require('../constants.js');
const to_var = str => str.split('-').join('_');
const formatted_supported_exports = SUPPORTED_WORKSPACE_EXPORTS.map(str => [str.split('.')[0], str]);
...
const main = () => {
    formatted_supported_exports.forEach(([name, import_name = name]) => {
        ...
        const module_exists = (dir) => fs.existsSync(path.resolve(APP_PATH, `workspaces/${dir}/${import_name}`));
        ...
        const export_contents = `import { ${name} } from '../utils/strip_build';\n\n` +
            `${enabled_workspaces
            ...
            `};\n`;
        fs.writeFileSync(path.resolve(WORKSPACES_CODEGEN_PATH, `webpack.${pluralize_suffix(name)}.ts`), export_contents);
    });
};
main();
